...
let allCaps = [runners.map(Upper => Upper.first_name.toUpperCase())];
console.log(allCaps); 
...
let ticketPriceTotal = runners.reduce((total, tickets) => {
    return total += tickets.donation;
}, 0);
console.log(ticketPriceTotal);
//You can name these two parameters anything.

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.
// Problem 1
//Tanner Branton has been found to be abusing drugs to enhance his performance, so we need to remove him from the race.
let noDrugs = runners.filter(function(item) {
    return item !== "Tanner";
});
console.log(noDrugs);
